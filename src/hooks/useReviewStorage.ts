...
import { GameReview, Period, CommentRecord } from '@/types/game-review';
...
interface UseReviewStorageReturn {
  ...
  setRecords: React.Dispatch<React.SetStateAction<Record<Period, CommentRecord[]>>>;
  gameInfo: {
    gameId: string;
    homeTeamName: string;
    awayTeamName: string;
...
export function useReviewStorage(): UseReviewStorageReturn {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (data) {
        ...
        console.log(`[AVC Storage] Loaded review for video ID: ${videoId}`, savedReview);
      }
    } catch (error) {
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.log(`[AVC Storage] Saved review for video ID: ${videoId}`, gameReview);
    } catch (error) {
